...
class CallManager {
  ...
  showIncomingCallPrompt(callerDisplayName, callerAvatar, callType = 'audio', callerId = null, callerSocketId = null, signalData = null, callerPhone = '') {
    // Check Privacy Settings (Always allow demo users and registered contacts)
    const videoPrivacy = localStorage.getItem('gitpit_video_call_privacy') || 'everyone';
    const isDemoCaller = ['user_9811122334', 'user_9822233445', 'user_9833344556', 'user_9844455667'].includes(callerId);

    if (callType === 'video' && videoPrivacy === 'nobody' && !isDemoCaller) {
    ...
    if (window.ChatterApp && window.ChatterApp.showInAppBanner) {
      window.ChatterApp.showInAppBanner(
        `Incoming ${callType === 'video' ? '📹 Video' : '📞 Audio'} Call`,
        `${callerDisplayName} is calling you...`,
        callerAvatar,
        () => {
          const prompt = document.getElementById('incoming-call-modal');
  ...
  async initLocalStream(includeVideo = false) {
    ...
    if (includeVideo) {
      // Tier 1: Try high-def with preferred facing mode
      try {
        ...
        console.warn('[CAMERA] HD constraint failed, trying basic video:', e1.message);
        // Tier 2: Try basic video constraints (works universally across laptops & webcams)
        try {
          stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: true });
        } catch (e2) {
          console.warn('[CAMERA] Basic video failed, falling back to audio only:', e2.message);
          // Tier 3: Audio only
    ...
    this.isCameraOff = !includeVideo;

    const localVideo = document.getElementById('local-video-feed');
    const localPip = document.getElementById('local-video-pip-wrapper');
    ...
    if (localVideo) {
      ...
      localVideo.play().catch(e => console.warn('localVideo play notice:', e));
    }
    if (localPip) {
      localPip.style.display = includeVideo ? 'block' : 'none';
    }
    if (cameraOffOverlay) {
      cameraOffOverlay.classList.toggle('active', !includeVideo);
    }
    if (localFallback) {
      localFallback.style.display = includeVideo ? 'none' : 'flex';
    }
