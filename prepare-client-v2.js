...
function injectScripts(filePath) {
  ...
  console.log(`[PREPARE V11] injected v1.0.4 repair scripts into ${filePath}`);
}

function bumpCache(filePath) {
  if (!fs.existsSync(filePath)) return;
  let sw = fs.readFileSync(filePath, 'utf8');
  sw = sw.replace(/const CACHE_NAME = 'gitpit-web-v[^']+';/, "const CACHE_NAME = 'gitpit-web-v1.0.12';");
  sw = sw.replace(/\s*'\.\/js\/message-routing-v3\.js',?/g, '');
  for (const asset of ['./js/identity-routing-v2.js','./js/ui-stability-v4.js','./js/contact-refresh-privacy-v4.js','./js/contact-directory-v5.js','./js/message-delivery-v4.js','./js/call-reliability-v5.js','./js/status-reliability-v6.js','./js/email-compose-v7.js','./js/meeting-invites-v8.js']) {
...
for (const name of ['identity-routing-v2.js','ui-stability-v4.js','contact-refresh-privacy-v4.js','contact-directory-v5.js','message-delivery-v4.js','call-reliability-v5.js','status-reliability-v6.js','email-compose-v7.js','meeting-invites-v8.js']) mirrorPublicScript(name);
...
console.log('[PREPARE V11] GitPit v1.0.4 client preparation complete');
