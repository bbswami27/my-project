...
function injectScripts(filePath) {
  ...
  console.log(`[PREPARE V16] injected v1.0.4 repair scripts into ${filePath}`);
}

function bumpCache(filePath) {
  if (!fs.existsSync(filePath)) return;
  let sw = fs.readFileSync(filePath, 'utf8');
  sw = sw.replace(/const CACHE_NAME = 'gitpit-web-v[^']+';/, "const CACHE_NAME = 'gitpit-web-v1.0.17';");
...
console.log('[PREPARE V16] GitPit v1.0.4 client preparation complete');
