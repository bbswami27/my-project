...
// Remove every legacy repair tag from HTML. Only the authenticated loader is injected.
const repairNames=[
  ...
  'responsive-ui-v14.js','safe-ui-final-v20.js','post-auth-repairs-v15.js'
];

const runtimeNames=[
  'contact-native-v16.js','message-delivery-v4.js','call-reliability-v5.js','status-reliability-v6.js',
  'meeting-invites-v8.js','screen-share-recipient-v9.js','safe-ui-final-v20.js','post-auth-repairs-v15.js'
];
...
function prepareHtml(filePath){
  ...
  const bootstrap='<script src="js/post-auth-repairs-v15.js?v=110"></script>';
  html=html.replace(/\s*<script src="js\/post-auth-repairs-v15\.js(?:\?[^\"]*)?"><\/script>\s*/g,'\n');
  html=html.replace('</body>',`  ${bootstrap}\n</body>`);
  fs.writeFileSync(filePath,html,'utf8');
  console.log(`[PREPARE V22] v1.1.0 production client prepared in ${filePath}`);
}

function bumpCache(filePath){
  if(!fs.existsSync(filePath))return;
  let sw=fs.readFileSync(filePath,'utf8');
  sw=sw.replace(/const CACHE_NAME = 'gitpit-web-v[^']+';/,"const CACHE_NAME = 'gitpit-web-v1.1.0';");
  for(const name of repairNames){
    const escaped=`./js/${name}`.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
    sw=sw.replace(new RegExp(`\\s*'${escaped}',?`,'g'),'');
  }
  // Only bootstrap is precached. Runtime modules load after authentication from packaged assets/network.
...
console.log('[PREPARE V22] GitPit v1.1.0 complete production preparation');
